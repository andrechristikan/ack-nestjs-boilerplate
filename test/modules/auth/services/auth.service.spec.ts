...
describe('AuthService', () => {
    const authSocialService = {
        verifyApple: vi.fn<AuthSocialService['verifyApple']>(),
        verifyGoogle: vi.fn<AuthSocialService['verifyGoogle']>(),
    } satisfies Pick<AuthSocialService, 'verifyApple' | 'verifyGoogle'>;
    const sessionCacheService = {
        getLogin: vi.fn<SessionCacheService['getLogin']>(),
    } satisfies Pick<SessionCacheService, 'getLogin'>;
    ...
    beforeEach(async () => {
        ...
        const moduleRef: TestingModule = await Test.createTestingModule({
            providers: [
                AuthService,
                { provide: AuthSocialService, useValue: authSocialService },
    ...
    describe('JWT strategy validation', () => {
        it.each([
            ...
            ['refresh', { ...refreshPayload, sessionId: '' }],
        ...
        it('returns a refresh payload bound to the current session jti', async () => {
            sessionCacheService.getLogin.mockResolvedValue({
                userId: 'user-id',
                sessionId: 'session-id',
                expiredAt: new Date('2026-02-01T00:00:00.000Z'),
    ...
    describe('JWT guard validation', () => {
        ...
        it('maps a missing refresh principal to the refresh-token exception', () => {
            expect(() =>
