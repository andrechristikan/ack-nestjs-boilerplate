...
describe('AuthJwtService', () => {
    ...
    beforeEach(async () => {
        ...
        service = moduleRef.get(AuthJwtService);
        jwtService = moduleRef.get(JwtService);
    });

    it('signs access and refresh tokens with separate credentials and policies', () => {
        ...
        expect(refreshDecoded).toMatchObject({
            header: { alg: 'ES512', kid: 'refresh-kid' },
            payload: { sub: 'user-id', jti: 'refresh-jti' },
        });
        expect(
            service.validateAccessToken('user-id', 'access-jti', accessToken)
        ).toBe(true);
        expect(
            service.validateRefreshToken('user-id', 'refresh-jti', refreshToken)
        ).toBe(true);
        expect(
            service.validateRefreshToken('user-id', 'access-jti', accessToken)
        ).toBe(false);
        expect(
            service.validateAccessToken('user-id', 'refresh-jti', refreshToken)
        ).toBe(false);
    });
    ...
    it('rotates both tokens while retaining the session and old refresh deadline', () => {
