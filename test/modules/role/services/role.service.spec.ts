...
describe('RoleService', () => {
    const roleRepository = {
        existByName: vi.fn<RoleRepository['existByName']>(),
        existById: vi.fn<RoleRepository['existById']>(),
        create: vi.fn<RoleRepository['create']>(),
        used: vi.fn<RoleRepository['used']>(),
        delete: vi.fn<RoleRepository['delete']>(),
    } satisfies Pick<
        RoleRepository,
        'existByName' | 'existById' | 'create' | 'used' | 'delete'
    >;
    const roleUtil = {
    ...
    it('rejects creation when a role name already exists', async () => {
        roleRepository.existByName.mockResolvedValue(role);
    ...
    it('rejects deletion while the role is assigned', async () => {
        roleRepository.existById.mockResolvedValue(role);
        roleRepository.used.mockResolvedValue({ id: role.id });
