...
describe('UserVerificationService', () => {
    const userVerificationRepository = {
        findOneActiveByVerificationEmailToken:
            ...
            >(),
        verifyEmail: vi.fn<UserVerificationRepository['verifyEmail']>(),
        findOneLatestByVerificationEmail:
            vi.fn<
                UserVerificationRepository['findOneLatestByVerificationEmail']
            >(),
        requestVerificationEmail:
            vi.fn<UserVerificationRepository['requestVerificationEmail']>(),
    } satisfies Pick<
    ...
    beforeEach(async () => {
        ...
        userVerificationRepository.requestVerificationEmail.mockResolvedValue(
            user
        );
        userVerificationRepository.verifyEmail.mockResolvedValue(verification);
        ...
        service = moduleRef.get(UserVerificationService);
    });
    ...
    describe('verifyEmail', () => {
        it('consumes a valid email verification token and sends the confirmation notification', async () => {
            ...
            expect(userVerificationRepository.verifyEmail).toHaveBeenCalledWith(
                verification.id,
                user.id,
                requestLog
            );
        ...
        it('throws UserTokenInvalidException when the token is absent or expired', async () => {
            ...
            expect(
                userVerificationRepository.verifyEmail
            ).not.toHaveBeenCalled();
        });
    });
    ...
    describe('sendVerificationEmail', () => {
        it('stores a new email verification and sends the encrypted link', async () => {
            ...
            expect(
                userVerificationRepository.requestVerificationEmail
            ).toHaveBeenCalledWith(
                user.id,
                user.email,
