...
describe('UserTwoFactorService', () => {
    ...
    const userLoginService = {
        handleTwoFactorValidation:
            vi.fn<UserLoginService['handleTwoFactorValidation']>(),
        createTokenAndSession:
            vi.fn<UserLoginService['createTokenAndSession']>(),
        revokeAllSessions: vi.fn<UserLoginService['revokeAllSessions']>(),
    } satisfies Pick<
        UserLoginService,
        | 'handleTwoFactorValidation'
        | 'createTokenAndSession'
    ...
    const authTwoFactorService = {
        setupTwoFactor: vi.fn<AuthTwoFactorService['setupTwoFactor']>(),
        generateBackupCodes:
            vi.fn<AuthTwoFactorService['generateBackupCodes']>(),
    } satisfies Pick<
        AuthTwoFactorService,
        'setupTwoFactor' | 'generateBackupCodes'
    >;
    const authCacheService = {
        getChallenge: vi.fn<AuthCacheService['getChallenge']>(),
        clearChallenge: vi.fn<AuthCacheService['clearChallenge']>(),
        clearLockTwoFactorAttempt:
            vi.fn<AuthCacheService['clearLockTwoFactorAttempt']>(),
    } satisfies Pick<
        AuthCacheService,
    ...
    let service: UserTwoFactorService;
    ...
    beforeEach(async () => {
        ...
        service = moduleRef.get(UserTwoFactorService);
    });
    ...
    describe('disableTwoFactor', () => {
        it('validates the factor before revoking sessions and disabling 2FA', async () => {
            ...
            userTwoFactorRepository.disableTwoFactor.mockImplementation(
                async () => {
                    order.push('disable');
                    return user;
                }
            ...
            expect(
                userTwoFactorRepository.disableTwoFactor
    ...
    describe('resetTwoFactorByAdmin', () => {
        it('resets another active user, clears attempt locks, and sends the notification', async () => {
            ...
            expect(
                userTwoFactorRepository.resetTwoFactorByAdmin
