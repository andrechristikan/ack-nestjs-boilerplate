...
all 138 controller methods. Each method receives a behavioral test, not only a route-mounted
smoke assertion.
...
The database bootstrap applies the current Prisma schema and seeds the baseline roles, policies,
feature flags, countries, term policies, and API keys. E2E fixtures are created through explicit
test support factories. State-changing cases verify database state through the real Prisma client.
...
- deterministic user, role, API key, workspace, project, session, device, policy, term-policy,
  invite, join-request, and notification fixtures;
- database reset and fixture cleanup;
...
6. Cover multipart upload, CSV import/export, presigning, and file response behavior.
7. Add the negative authorization matrix and enforce one behavioral test for every matrix entry.
8. Add the optional compiled-process smoke suite.

## Acceptance Criteria

- All 138 controller methods appear in the matrix and receive behavioral coverage.
