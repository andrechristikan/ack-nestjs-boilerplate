...
export class TermsPolicyAcceptedGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      const latestPolicy = await this.termsPolicyService.findLatestPublishedByType(policyType, language);
      ...
      const userAcceptedPolicy = await this.termsPolicyAcceptanceService.findOneAcceptedByUser(
        user.sub,
        latestPolicy.type,
        language,
      );
      ...
      if (options.requireLatestVersion) {
        const isLatestVersion = userAcceptedPolicy.policy.version >= latestPolicy.version;
