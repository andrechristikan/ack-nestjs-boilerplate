...
export class UserService {
    ...
    async loginWithCredential({
        email,
        password,
    }: UserLoginRequestDto): Promise<AuthTokenResponseDto> {
    ...
    async signUp({
        ...
        const [role, country, exists] = await Promise.all([
            this.roleRepository.findOneByName('user'),
            this.countryRepository.findOneByAlpha2Code(countryAlpha2Code),
    ...
    createRandomUsername(): string {
        const suffix = this.helperService.randomString(10);
