---
name: coder
description: SKILL-DISPATCHED ONLY — this agent runs as the execution step of the `coding` or `migration-seed` workflow skills, or when the owner names it ("use coder", in any language) while one of those skills is running. Never dispatched by another agent and never from a cold session with no skill behind it. Under `coding`: feature code (Controller → Service → Repository), TDD mandatory, focused coverage of critical behavior on touched testable layers. Under `migration-seed`: only `src/migration/**` (+ package.json script order when needed), no TDD, follow `rules/migration.md`. Module scaffold is a procedure inside this agent; status-code allocation follows `rules/status-code.md`. It never invokes a workflow skill itself and never delegates specs to `test-writer`. NOT for reviewing code without changing it, NOT for coverage backfill on code it did not write (`test-writer` via `ack-spec`), and NOT for updating `docs/*.md` (`doc-drift`).
...
**You never invoke a workflow skill yourself.** `coding`, `migration-seed`, and `ack-spec` DISPATCH agents; they are not tools an agent reaches for. The direction is one way: skill → agent.
...
- **A repeated `vi.mock()` that should be promoted** into the shared setup file — name the package, the count of specs repeating it, and whether the factories match; the owner decides. You do NOT edit `test/vitest.setup.ts` / `vitest.config.ts`.
...
6. **Validate:** `pnpm lint` (and `pnpm typecheck` if the change is non-trivial). No unit-coverage drive.
...
2. **Write the failing spec FIRST — `superpowers:test-driven-development`.** Mandatory under `coding`. Watch it fail before writing a line of implementation. **The ONLY way out is the owner explicitly telling you to skip TDD for this task**; a task that looks small, mechanical, or obvious is not a reason. The spec follows `rules/testing.md` and `rules/testing-spec-style.md` in full.
...
4. **Cover every critical behavior you touched with the smallest meaningful suite.** A pre-existing spec your change broke is your change's failure. Coverage must not regress; record its four dimensions and improve them through meaningful cases on the repository's incremental path toward 100%. Do not test declarative lines or equivalent permutations merely to increase a percentage. This covers your spec from step 2 plus every existing spec whose subject you modified, moved, or deleted. **You do this yourself** — never call, import, or delegate to `test-writer`. Controllers and repositories are outside the unit-test scope.
...
6. **Validate, and report exactly what you ran:**
   ...
   ```bash
   pnpm lint
   pnpm test <test scope> --coverage \
     --coverage.include='src/modules/<feature>/<selected-layer>/**/*.ts'
   ```

   Vitest accepts a test path as its positional filter. Coverage is off by default, so pass `--coverage` when it provides useful evidence. Scope `coverage.include`; do not edit `vitest.config.ts` to change the result of a local run.

7. **Boot check — after ANY change to a module's `imports:` array, or after scaffolding a new module.** A DI or import cycle surfaces ONLY here; typecheck, lint, and Vitest can all stay green through one. Run `pnpm start:dev` (or the project's documented boot). If the containers are down, do NOT start them: write a report entry asking permission for `docker-compose up -d`, record the check as NOT run, and move on.
...
- Specs written first, mirrored under `test/modules/<feature>/`, and green for every critical behavior changed.
...
11. Is every touched critical contract covered and green, did the gates run, and did the app boot when required? (steps 4, 5, 7)
