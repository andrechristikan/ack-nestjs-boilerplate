...
**`doc-drift` is the ONLY agent that may write `docs/*.md`.** `coder`, `test-writer`, `reviewer-flow`, and `pr-doc-writer` are forbidden from the whole tree. A stale doc they notice is named in their hand-back for `doc-drift` to apply. The owner may still edit `docs/` directly; no agent may, except `doc-drift`.
...
| backfilling or repairing critical unit specs for one named module, then ratcheting meaningful coverage toward 100% — `test/` only, no `src/` behavior change | `ack-spec` |
| a pull-request title + description for the current branch (`generated/docs/pr-<feature>.md`) | `pr-doc` |

`coding` does **not** invoke `ack-spec`, `migration-seed`, or `pr-doc`. They are parallel workflow skills. Under `coding`, TDD is mandatory and lives **inside `coder`** (failing spec first — the same head watches red turn green). `migration-seed` also dispatches `coder`, but for seeds only — no TDD, no flow review. `ack-spec` is for backfill/repair of specs against code that already exists; it permits no `src/` change. `pr-doc` is owner-triggered only and is the single door into `pr-doc-writer`.
