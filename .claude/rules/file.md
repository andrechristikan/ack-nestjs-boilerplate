...
- **The declared extension and the bytes must both agree with the allow-list.** `FileExtensionPipe` reads the extension from `originalname`, then sniffs the buffer through `FileService.sniffExtensionFromBuffer`. A sniffed type is accepted when `FileExtensionSignatures` (`file.constant.ts`) maps it onto a member of the allow-list. `csv` is the signature-less member: a sniff that returns nothing is accepted for it, and rejected for every other member. `hbs` is not in that table — it is not an upload. Adding an uploadable extension to a group enum means adding its row to that table.
